...
next patch tag, waits for GitHub Build and Release, then updates rgo-vps prod.

Environment:
  COMMIT_MESSAGE   Commit message when no positional message is passed.
  SSH_TARGET       Deployment SSH target. Default: rgo@rgo-vps
...
ssh_target="${SSH_TARGET:-rgo@rgo-vps}"
...
cd ~/.config/home
git pull --ff-only
printf "openpost image config: "
nix eval .#nixosConfigurations.rgo-vps.config.vps.openpost.image --raw
printf "\n"
nh os switch
sudo podman pull ghcr.io/rodrgds/openpost:latest
sudo systemctl restart podman-openpost.service
for i in $(seq 1 45); do
  if curl -fsS http://127.0.0.1:8090/api/v1/ready >/tmp/openpost-ready.json; then
    cat /tmp/openpost-ready.json
    printf "\n"
    break
  fi
  sleep 2
  if [ "$i" -eq 45 ]; then
    sudo journalctl -u podman-openpost.service -n 120 --no-pager
    exit 1
  fi
done
sudo podman ps --format "{{.Names}} {{.Image}} {{.Status}}" | grep openpost
sudo podman image inspect ghcr.io/rodrgds/openpost:latest --format "local latest id={{.Id}} digest={{index .RepoDigests 0}} created={{.Created}}"
