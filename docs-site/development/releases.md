...
| --- | --- | --- |
| Backward-compatible fix or maintenance | Patch | `v1.27.8` → `v1.27.9` |
| Backward-compatible feature | Minor | `v1.27.8` → `v1.28.0` |
| Breaking API or product change | Major | `v1.27.8` → `v2.0.0` |
...
The code shipped as `v1.1.22` therefore maps to `v1.27.7`. The versioning and documentation correction is the next patch-only release, so the corrected published line starts at `v1.27.8`. Existing tags remain immutable; future releases continue from that version using the rules above.
